(() => {
  ...
  const owner_id     = getUid(); // <- your table column is owner_id
  ...
  // Build insert payload using your exact column names
  const payload = {
    owner_id,           // <— IMPORTANT
    product,
    account_type,
  ...
  setLoading(true);
  const { error } = await supabase.from('stocks').insert([payload]); // <- direct insert, no RPC
  setLoading(false);
