...
(() => {
  ...
  const setLoading = (on) => { const L=$(".loading-overlay"); if(!L) return; L.classList.toggle("hidden", !on); if(!on) L.style.pointerEvents="none"; };
  ...
  // --- duration helper (FIXED)
  const durMs = (code) => {
    if (!code) return 0;
    if (/^\d+d$/.test(code)) return parseInt(code,10) * 86400000;
    if (/^\d+m$/.test(code)) {
      const m = parseInt(code,10);
      return (m===12 ? 365 : 30*m) * 86400000;
    }
    return 0;
  };

  async function boot(){
    // ---------- wire login toggles FIRST
    ...
    const supabase = (APP.url && APP.key && window.supabase)
      ? window.supabase.createClient(APP.url, APP.key)
      : null;
    ...
    async function loadAccountTypes() {
      try { if(!supabase) throw 0; const {data,error}=await supabase.from("account_types").select("label").order("label"); if(error) throw error; if(data?.length) return data.map(r=>r.label); } catch {}
      return APP.ACCOUNT_TYPES || [];
    }
    async function loadDurations() {
      try { if(!supabase) throw 0; const {data,error}=await supabase.from("durations").select("label,code,seq").order("seq",{ascending:true}); if(error) throw error; if(data?.length) return data.map(r=>[r.label,r.code]); } catch {}
    ...
    async function ownerRenderRecords(){
      ...
      $$(".btnRecEdit",tbody).forEach(btn=>btn.addEventListener("click",async()=>{
        ...
        const cur={ product:tr.children[1].textContent.trim(), type:tr.children[2].textContent.trim(),
          expires_at:tr.children[4].textContent.trim(), buyer_link:tr.children[6].textContent.trim(),
    ...
    async function adminFillFormOptions(){
      ...
      prodSel.onchange=refresh;
      catSel && (catSel.onchange=adminFillFormOptions);
    }
    ...
    if(r && u){ r==="owner" ? (showOwner(),ownerRenderStocks(),ownerRenderRecords()) : (showAdmin(),adminRefreshAll()); }
    else { showLogin(); }
