/* app.js — AiaxStock (v2025-11-08.31) */
(() => {
  // DOM helpers
  ...
  // duration → ms
  const durMs = (code) => /^\d+d$/.test(code||"") ? parseInt(code,10)*86400000
                 : /^\d+m$/.test(code||"") ? (parseInt(code,10)===12?365:30*parseInt(code,10))*86400000
                 : 0;

  async function boot(){
    // 1) Wire login toggles FIRST
    ...
    async function loadAccountTypes(){
      try{ if(!supabase) throw 0;
        ...
        if(error) throw error; if(data?.length) return data.map(r=>r.label);
      }catch{} return APP.ACCOUNT_TYPES||[];
    }
    async function loadDurations(){
      try{ if(!supabase) throw 0;
        const {data,error}=await supabase.from("durations").select("label,code,seq").order("seq",{ascending:true});
        if(error) throw error; if(data?.length) return data.map(r=>[r.label,r.code]);
      }catch{} return APP.DURATIONS||[];
    }
    ...
    // OWNER features (same as your previous working version)
    async function ownerAddStock(){
      if(!supabase) return alert("Missing config.js");
    ...
    async function ownerRenderStocks(){
      ...
      $$(".btnRemove",tbody).forEach(b=>b.addEventListener("click",async()=>{
        const id=b.closest("tr").dataset.id; if(!confirm(`Delete stock #${id}?`)) return;
      ...
      $$(".btnEdit",tbody).forEach(b=>b.addEventListener("click",async()=>{
        ...
        setLoading(false);
        if(error){ alert("Edit failed"); console.error(error); return; } ownerRenderStocks();
      }));
    ...
    async function ownerAddRecord(){
      ...
      $("#recWarranty")?.checked=false; ownerRenderRecords();
    }
    ...
    async function ownerRenderRecords(){
      ...
      tbody.innerHTML=rows.map(r=>`
        <tr data-id="${r.id}">
          <td>${r.id}</td><td>${r.product??""}</td><td>${r.account_type??""}</td><td>${fmtDT(r.created_at)}</td>
          <td>${fmtDT(r.expires_at)}</td><td>${r.admin_uuid?r.admin_uuid.slice(0,8):""}</td>
      ...
      $$(".btnRecEdit",tbody).forEach(btn=>{
        btn.addEventListener("click",async()=>{
          ...
          const cur={product:tr.children[1].textContent.trim(),type:tr.children[2].textContent.trim(),
            expires_at:tr.children[4].textContent.trim(),buyer_link:tr.children[6].textContent.trim(),price:tr.children[7].textContent.trim()};
          ...
          toast("Record updated"); ownerRenderRecords();
        });
      });
      $$(".btnRecDel",tbody).forEach(btn=>{
        btn.addEventListener("click",async()=>{
          ...
          setLoading(true); const {error}=await supabase.from("sales").delete().eq("id",id).eq("owner_uuid",getUid());
          setLoading(false); if(error){ console.error(error); return alert("Delete failed"); }
          ownerRenderRecords();
        });
      });
    ...
    async function adminAvailable(){
      try{ if(!supabase) throw 0;
        const {data,error}=await supabase.from("stocks_available_for_admin")
          .select("product,account_type,duration_code,total_qty").order("product");
        if(error) throw error; if(data?.length) return data;
      }catch{}
      if(!supabase) return [];
      const {data,error}=await supabase.from("stocks")
        .select("product,account_type,duration_code,quantity,archived").gt("quantity",0).eq("archived",false);
      if(error){ console.error(error); return []; }
      const map=new Map();
    ...
    async function adminFillFormOptions(){
      ...
      function refresh(){ const p=prodSel.value; const sub=avail.filter(r=>r.product===p);
    ...
    // CONTINUE buttons (final binding)
    ...
    // auto-view by session
    const r=getRole(), u=getUid();
    if(r && u){ r==="owner" ? (showOwner(), ownerRenderStocks(), ownerRenderRecords())
