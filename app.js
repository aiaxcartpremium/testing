/* app.js — AiaxStock (v2025-11-08.31 “login click fix + clean wiring”) */
(() => {
  // ---------- tiny DOM helpers
  ...
  async function boot(){
    // 0) Make sure any buttons we need are not <button type="submit">
    [
      "#btnLoginOwner","#btnLoginAdmin","#continueOwner","#continueAdmin",
      "#goToAdmin","#goToOwner",".btnLogout",
      "#oaAddBtn","#btnOwnerRefresh","#btnOwnerPurge","#btnAddRecord","#getAccountBtn",
      ".tab",".chip"
    ].forEach(sel => $$(sel).forEach(b => b.type = "button"));

    // 1) Bind LOGIN UI FIRST so it always works
    const btnLoginOwner = $("#btnLoginOwner");
    ...
    btnLoginOwner?.addEventListener("click", (e)=>{
      e.preventDefault();
    ...
    async function ownerAddStock(){
      ...
      if(premiumed_at_raw){
        const d=new Date(premiumed_at_raw); if(!isNaN(d)) payload.premiumed_at=d.toISOString();
      }
    ...
    async function ownerRenderStocks(){
      ...
      tbody.innerHTML=data.map(r=>`
        <tr data-id="${r.id}">
          <td>${r.id}</td><td>${r.product}</td><td>${r.account_type}</td><td>${r.duration_code}</td><td>${r.quantity}</td>
          <td>${fmtDT(r.premiumed_at)}</td><td>${fmtDT(r.created_at)}</td><td>${r.auto_expire_days ?? ""}</td>
    ...
    async function adminFillFormOptions(){
      const catSel=$("#catSelectAdmin"); const prodSel=$("#productSelectAdmin");
      const typeSel=$("#typeSelectAdmin"); const durSel=$("#durSelectAdmin");
    ...
    // ---------- Continue buttons (final step of login)
    $("#continueOwner")?.addEventListener("click",(e)=>{
      e.preventDefault();
    ...
    $("#continueAdmin")?.addEventListener("click",(e)=>{
      e.preventDefault();
