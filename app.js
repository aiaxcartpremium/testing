/* app.js — AiaxStock (stable 2025-11-08) */
(() => {
  // small DOM helpers
  ...
  // ---------- SESSION HELPERS (no tricky assignments here)
  const SKEY_ROLE = "aiax.role";
  const SKEY_UID  = "aiax.uid";

  function setSess(role, uid) {
    if (!role || !uid) return;
  ...
  async function boot(){
    // ---- bind LOGIN toggles FIRST so buttons are always clickable
    ...
    btnContOwner?.addEventListener("click", (e)=>{
      ...
      setSess("owner", id);
      showOwner();
      ownerRenderStocks(); ownerRenderRecords();
    });
    ...
    // ---- Supabase client (if config present)
    if (!APP.url || !APP.key) {
      console.warn("config.js missing or incomplete. Owner/Admin UI will still render.");
    }
    const supabase = (APP.url && APP.key) ? window.supabase.createClient(APP.url, APP.key) : null;

    // ====== DATA + RENDER FUNCTIONS (unchanged logic) ======
    let ALL_PRODUCTS = [];
    let CUR_CAT = null;
    ...
    async function loadProducts() {
      try {
        if(!supabase) throw new Error("no sb");
    ...
    async function ownerPurgeExpired(){
      ...
      const { data, error } = await supabase
        .from("stocks").select("id,created_at,auto_expire_days,quantity,archived,owner_id")
    ...
    async function ownerRenderRecords(){
      ...
      try {
        ...
        $("#btnExportCSV")?.addEventListener("click", ()=>{
          ...
          const head = Object.keys(rows2[0]);
          const esc = v => v==null ? "" : /[",\n]/.test(String(v)) ? `"${String(v).replace(/"/g,'""')}"` : String(v);
          const csv = [head.join(","), ...rows2.map(r=>head.map(k=>esc(r[k])).join(","))].join("\n");
    ...
    async function adminFillFormOptions(){
      ...
      products.forEach(p=>{
        ...
        o.value=p; o.textContent=ALL_PRODUCTS.find(x=>x.key===p)?.label || p; prodSel.appendChild(o);
      });
  ...
  document.addEventListener("DOMContentLoaded", () => {
    try { boot(); }
    catch(e){ alert(`JS error:\n${e.name}: ${e.message}`); console.error(e); }
  });
})();
