/* app.js — AiaxStock (v2025-11-08.23) */
(() => {
  // tiny DOM helpers
  ...
  // session
  const SKEY_ROLE="aiax.role", SKEY_UID="aiax.uid";
  ...
  // durations
  const durMs = (code) => (/^\d+d$/.test(code)?parseInt(code,10)*86400000 : /^\d+m$/.test(code)?( (m=> (m===12?365:30*m)*86400000)(parseInt(code,10)) ) : 0);

  async function boot(){
    // --- bind login toggles FIRST
    ...
    btnOwner?.addEventListener("click",e=>{e.preventDefault(); cardAdmin?.classList.add("hidden"); cardOwner?.classList.remove("hidden"); inputOwner?.focus();});
    btnAdmin?.addEventListener("click",e=>{e.preventDefault(); cardOwner?.classList.add("hidden"); cardAdmin?.classList.remove("hidden"); inputAdmin?.focus();});

    // --- config + client
    const APP = window.APP || {};
    const supabase = (APP.url && APP.key) ? window.supabase.createClient(APP.url, APP.key) : null;
    const isOwner = uid => APP.ownerId && norm(uid)===norm(APP.ownerId);
    const isAdmin = uid => Array.isArray(APP.admins) && APP.admins.map(norm).includes(norm(uid));

    // --- continue buttons
    const btnContOwner=$("#continueOwner"), btnContAdmin=$("#continueAdmin");
    ...
    // shared state
    let ALL_PRODUCTS=[]; let CUR_CAT=null;

    // --- options helpers
    const fillSelect=(sel,items)=>{const el=$(sel); if(!el) return; el.innerHTML=""; (items||[]).forEach(it=>{let label,value;
      if(Array.isArray(it)) [label,value]=it; else if(typeof it==="string") label=value=it;
      else if(it && it.label!=null && it.key!=null){label=it.label; value=it.key;} else return;
      const o=document.createElement("option"); o.value=value; o.textContent=label; el.appendChild(o);});
    };

    async function loadProducts(){
      try{ if(!supabase) throw 0;
        const {data,error}=await supabase.from("products").select("key,label,category").order("label");
        if(error) throw error; if(data?.length) return data;
      }catch{}
      return (APP.PRODUCTS||[]).map(x=> typeof x==="string" ? ({key:x.toLowerCase(),label:x,category:"Entertainment"}) : x);
    }
    async function loadAccountTypes(){
      try{ if(!supabase) throw 0; const {data,error}=await supabase.from("account_types").select("label").order("label");
        if(error) throw error; if(data?.length) return data.map(r=>r.label);}catch{} return APP.ACCOUNT_TYPES||[];
    }
    async function loadDurations(){
      try{ if(!supabase) throw 0; const {data,error}=await supabase.from("durations").select("label,code,seq").order("seq",{ascending:true});
        if(error) throw error; if(data?.length) return data.map(r=>[r.label,r.code]);}catch{} return APP.DURATIONS||[];
    }
    ...
    async function primeOptions(){
      ...
      fillSelect("#typeSelectAdmin",[]); fillSelect("#durSelectAdmin",[]); fillSelect("#catSelectAdmin",[]);

      const [prods,types,durs]=await Promise.all([loadProducts(),loadAccountTypes(),loadDurations()]);
      ALL_PRODUCTS=prods; buildCatBar();
    ...
    async function ownerAddStock(){
      ...
      if(profile_name) payload.profile_name=profile_name; if(pin) payload.pin=pin; if(notes) payload.notes=notes;
      if(premiumed_at_raw){const d=new Date(premiumed_at_raw); if(!isNaN(d)) payload.premiumed_at=d.toISOString();}
      if(auto_expire_raw && !isNaN(parseInt(auto_expire_raw,10))) payload.auto_expire_days=parseInt(auto_expire_raw,10);

      setLoading(true);
      const {error}=await supabase.from('stocks').insert([payload]);
      setLoading(false);
      if(error){console.error(error); return alert('Add stock failed: '+(error.message||'unknown'));}
      toast('Stock added');
      $('#oaQty').value='1'; ['oaEmail','oaPass','oaProfile','oaPin','oaNotes','oaPremiumedAt','oaAutoExpireDays'].forEach(id=>{const el=$('#'+id); if(el) el.value='';});
      ownerRenderStocks();
    }
    ...
    async function ownerRenderStocks(){
      ...
      $$(".btnRemove",tbody).forEach(b=>b.addEventListener("click",async()=>{
        ...
        setLoading(true); const {error}=await supabase.from("stocks").delete().eq("id",id); setLoading(false);
        if(error){alert("Remove failed"); console.error(error); return;} ownerRenderStocks();
      }));
      $$(".btnEdit",tbody).forEach(b=>b.addEventListener("click",async()=>{
        const tr=b.closest("tr"); const id=tr.dataset.id;
        const cur={quantity:parseInt(tr.children[4].textContent||"0",10), premiumed_at:tr.children[5].textContent, auto_expire_days:tr.children[7].textContent};
        ...
        setLoading(false); if(error){alert("Edit failed"); console.error(error); return;} ownerRenderStocks();
      }));
      $$(".btnArchive",tbody).forEach(b=>b.addEventListener("click",async()=>{
        const id=b.closest("tr").dataset.id; const {error}=await supabase.from("stocks").update({archived:true}).eq("id",id);
        if(error){alert("Archive failed"); console.error(error); return;} ownerRenderStocks();
      }));
      $$(".btnUnarchive",tbody).forEach(b=>b.addEventListener("click",async()=>{
        const id=b.closest("tr").dataset.id; const {error}=await supabase.from("stocks").update({archived:false}).eq("id",id);
        if(error){alert("Unarchive failed"); console.error(error); return;} ownerRenderStocks();
      }));
    }
    ...
    async function ownerPurgeExpired(){
      ...
      const {data,error}=await supabase
        ...
        .eq("owner_id",getUid()).eq("archived",false).gt("auto_expire_days",0);
    ...
    async function ownerAddRecord(){
      ...
      setLoading(true);
      const {data:rows,error:errFind}=await supabase.from("stocks").select("*")
      ...
      if(rows?.length){
        ...
        const {error:decErr}=await supabase.from("stocks").update({quantity:(s.quantity||1)-1}).eq("id",s.id).gt("quantity",0);
        decOk=!decErr;
      }
      const now=new Date();
      ...
      ["recBuyer","recPrice","recExtraDays"].forEach(id=>{const el=$("#"+id); if(el) el.value="";}); $("#recWarranty")?.checked=false;
      ownerRenderRecords();
    }
    ...
    async function ownerRenderRecords(){
      ...
      tbody.innerHTML=rows.map(r=>`
        <tr data-id="${r.id}">
          ...
          <td><button class="btn-outline btnRecEdit" type="button">Edit</button>
              <button class="btn-outline btnRecDel"  type="button">Delete</button></td>
        </tr>`).join("");

      $$(".btnRecEdit",tbody).forEach(btn=>btn.addEventListener("click",async()=>{
        const tr=btn.closest("tr"); const id=tr.dataset.id;
        const cur={product:tr.children[1].textContent.trim(),type:tr.children[2].textContent.trim(),expires_at:tr.children[4].textContent.trim(),
                   buyer_link:tr.children[6].textContent.trim(),price:tr.children[7].textContent.trim()};
        ...
        const priceIn=prompt("Price (blank to clear):",cur.price)||""; const price= priceIn?Number(priceIn):null;
        const addDaysStr=prompt("Add warranty days (0 to keep):","0")||"0"; const extraDays=parseInt(addDaysStr,10)||0;
        let expires=cur.expires_at?new Date(cur.expires_at):new Date(); if(extraDays>0) expires=addDays(expires,extraDays);
        setLoading(true);
        const {error}=await supabase.from("sales").update({product,account_type:type,buyer_link:buyer,price,expires_at:expires.toISOString()}).eq("id",id).eq("owner_uuid",getUid());
        setLoading(false); if(error){console.error(error); return alert("Update failed");} toast("Record updated"); ownerRenderRecords();
      }));
      $$(".btnRecDel",tbody).forEach(btn=>btn.addEventListener("click",async()=>{
        const tr=btn.closest("tr"); const id=tr.dataset.id; if(!confirm(`Delete record #${id}?`)) return;
        setLoading(true); const {error}=await supabase.from("sales").delete().eq("id",id).eq("owner_uuid",getUid()); setLoading(false);
        if(error){console.error(error); return alert("Delete failed");} ownerRenderRecords();
      }));
      ...
      $("#btnExportCSV")?.addEventListener("click",()=>{
        ...
        a.href=url; a.download="owner_records.csv"; a.click(); URL.revokeObjectURL(url);
      },{once:true});
    }
    ...
    async function adminAvailable(){
      ...
      if(error){console.error(error); return [];}
      const map=new Map();
      for(const r of data){const k=`${r.product}|${r.account_type}|${r.duration_code}`; map.set(k,(map.get(k)||0)+(r.quantity||0));}
    ...
    async function adminRenderAvailable(){
      ...
      body.innerHTML= rows.length ? rows.map(r=>`<tr><td>${r.product}</td><td>${r.account_type}</td><td>${r.duration_code}</td><td>${r.total_qty}</td></tr>`).join("")
                                  : `<tr><td colspan="4" class="muted">No data yet</td></tr>`;
    }

    async function adminFillFormOptions(){
      const catSel=$("#catSelectAdmin"), prodSel=$("#productSelectAdmin"), typeSel=$("#typeSelectAdmin"), durSel=$("#durSelectAdmin");
      ...
      const allowedKeys=new Set((formCat?ALL_PRODUCTS.filter(p=>(p.category||"Uncategorized")===formCat):ALL_PRODUCTS).map(p=>p.key));
      const products=uniq(avail.map(r=>r.product).filter(k=>allowedKeys.has(k)));
      prodSel.innerHTML=""; products.forEach(p=>{const o=document.createElement("option"); o.value=p; o.textContent=ALL_PRODUCTS.find(x=>x.key===p)?.label||p; prodSel.appendChild(o);});
      function refresh(){const p=prodSel.value; const sub=avail.filter(r=>r.product===p); typeSel.innerHTML=""; durSel.innerHTML="";
    ...
    async function adminGetAccount(){
      if(!supabase) return alert("Missing Supabase config.");
      const product=$("#productSelectAdmin")?.value; const type=$("#typeSelectAdmin")?.value; const duration=$("#durSelectAdmin")?.value;
      ...
      setLoading(true);
      const res=await supabase.rpc("get_account_v2",{p_admin:admin_uuid,p_product:product,p_type:type,p_duration:duration});
      setLoading(false);
      if(res.error) return alert("get_account failed: "+res.error.message);
      const data=res.data||[]; if(!data.length){$("#adminCreds").textContent="No matching stock."; return;}
      const r=data[0];
    ...
    // view routing
    function showOwner(){ if(getRole()!=="owner"){alert("Owners only."); return;}
      $("#viewLogin")?.classList.add("hidden"); $("#viewAdmin")?.classList.add("hidden"); $("#viewOwner")?.classList.remove("hidden");
      $("#goToOwner")?.classList.add("hidden"); $("#goToAdmin")?.classList.toggle("hidden",getRole()!=="owner");
    }
    function showAdmin(){ $("#viewLogin")?.classList.add("hidden"); $("#viewOwner")?.classList.add("hidden"); $("#viewAdmin")?.classList.remove("hidden");
      $("#goToOwner")?.classList.toggle("hidden",getRole()!=="owner"); $("#goToAdmin")?.classList.add("hidden");
    }
    function showLogin(){ $("#viewLogin")?.classList.remove("hidden"); $("#viewOwner")?.classList.add("hidden"); $("#viewAdmin")?.classList.add("hidden"); }
    ...
    tabs.forEach(t=>t.addEventListener("click",()=>{
      ...
      const target=t.dataset.target; $$(".tab-page").forEach(p=>p.classList.add("hidden")); $("#"+target)?.classList.remove("hidden");
    }));
