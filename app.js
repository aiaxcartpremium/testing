/* app.js — AiaxStock (stabilized login & overlay) */
(() => {
  // tiny helpers
  ...
  const setLoading = (on) => {
    ...
    L.style.pointerEvents="none"; // never block clicks
  };
  const fmtDT = (d) => d ? new Date(d).toLocaleString() : "";
  const addDays = (date, days) => new Date(date.getTime() + (days||0)*86400000);

  // simple session helpers (NO chained assignment)
  const SKEY_ROLE="aiax.role", SKEY_UID="aiax.uid";
  function setSess(role, uid){ sessionStorage.setItem(SKEY_ROLE, role); sessionStorage.setItem(SKEY_UID, uid); }
  function getRole(){ return sessionStorage.getItem(SKEY_ROLE); }
  function getUid(){ return sessionStorage.getItem(SKEY_UID); }
  function clearSess(){ sessionStorage.removeItem(SKEY_ROLE); sessionStorage.removeItem(SKEY_UID); }
  window.AiaxSession = { setSess, getRole, getUid, clearSess };
  ...
  async function boot(){
    ...
    [btnOwner,btnAdmin,$("#continueOwner"),$("#continueAdmin"),
     $("#oaAddBtn"),$("#btnOwnerRefresh"),$("#btnOwnerPurge"),
     $("#btnAddRecord"),$("#getAccountBtn")]
       .forEach(b=>b && (b.type="button"));
    ...
    // 2) Safe config + supabase
    const APP = window.APP || {};
    let supabase = null;
    try{
      if(APP.url && APP.key) supabase = window.supabase.createClient(APP.url, APP.key);
    }catch(e){ console.warn("Supabase init failed:", e); }

    const isOwner = uid => norm(uid) === norm(APP.ownerId||"");
    const isAdmin = uid => (APP.admins||[]).map(norm).includes(norm(uid));

    // ---- shared state for admin lists
    ...
    async function ownerAddStock(){
      if(!supabase) return alert("Backend not configured.");
    ...
    async function ownerAddRecord(){
      ...
      const { data:rows, error:errFind } = await supabase
        .from("stocks").select("*").eq("owner_id", getUid())
        .eq("product", product).eq("account_type", account_type)
      ...
      if(rows?.length){
        ...
        const { error:decErr } = await supabase.from("stocks")
          .update({ quantity:(s.quantity||1)-1 }).eq("id", s.id).gt("quantity", 0);
        decOk=!decErr;
      }
      ...
      const { error:insErr } = await supabase.from("sales").insert([{
        product, account_type, created_at: now.toISOString(),
        expires_at: expiresBase.toISOString(), admin_uuid:getUid(), owner_uuid:getUid(),
        buyer_link, price, warranty_days: withWarranty ? extraDays : 0
      }]);
      setLoading(false);
      if(insErr){ console.error(insErr); return alert("Insert failed"); }
      toast(decOk ? "Record added (stock decremented)" : "Record added");
    ...
    async function ownerRenderRecords(){
      ...
      tbody.innerHTML=rows.map(r=>`
        <tr data-id="${r.id}">
          ...
          <td><button class="btn-outline btnRecEdit">Edit</button> <button class="btn-outline btnRecDel">Delete</button></td>
        </tr>`).join("");
    ...
    async function adminAvailable(){
      ...
      const {data,error}=await supabase
        .from("stocks").select("product,account_type,duration_code,quantity,archived")
    ...
    async function adminFillFormOptions(){
      ...
      const allowed = new Set( (formCat ? ALL_PRODUCTS.filter(p=>(p.category||"Uncategorized")===formCat) : ALL_PRODUCTS).map(p=>p.key) );
      const products=uniq(avail.map(r=>r.product).filter(k=>allowed.has(k)));
    ...
    async function adminRenderMySales(){
      ...
      if(error){ console.error(error); tbody.innerHTML=`<tr><td colspan="6">Failed to load.</td></tr>`; return; }
      const rows=data||[];
      ...
      $$(".btnEditRec",tbody).forEach(b=>b.addEventListener("click", async ()=>{
        ...
        const price = priceStr==="" ? null : Number(priceStr);
        const {error:err}=await supabase.from("sales").update({ buyer_link, price }).eq("id",id);
        if(err){ alert("Update failed"); console.error(err); return; }
        adminRenderMySales();
      }));
    }
    ...
    document.addEventListener("click",(e)=>{
      ...
      if(t.id==="continueOwner"){
        const id=(inputOwner?.value||"").trim(); if(!isOwner(id)) return alert("UUID is not an Owner ID.");
        setSess("owner", id); showOwner(); ownerRenderStocks(); ownerRenderRecords();
      }else{
        const id=(inputAdmin?.value||"").trim(); if(!(isOwner(id)||isAdmin(id))) return alert("UUID is not an Admin ID.");
