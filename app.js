(() => {
  ...
  // convert duration code to ms (rough months=30d)
  const durMs = (code) => {
    if (!code) return 0;
    if (/^\d+d$/.test(code)) return parseInt(code)*86400000;
    if (/^\d+m$/.test(code)) return parseInt(code)*30*86400000;
    return 0;
  };

  // ── options
  async function loadProducts() {
    try { const {data,error} = await supabase.from("products").select("name").order("name"); if(error) throw error; if(data?.length) return data.map(r=>r.name); } catch {}
    return APP.PRODUCTS || [];
  }
  ...
  function ownerStocksSelect(showArchived){
    return supabase.from("stocks")
      .select("id,product,account_type,duration_code,quantity,premiumed_at,created_at,auto_expire_days,archived,owner_uuid")
      .eq("owner_uuid", getUid())
      .eq("archived", !!showArchived ? true : false); // when false: only unarchived
  }
  ...
  async function ownerRenderStocks(){
    ...
    $$(".btnEdit", tbody).forEach(b=>b.addEventListener("click", async ()=>{
      ...
      const cur = {
        ...
        account_type: tr.children[2].textContent,
        duration_code: tr.children[3].textContent,
  ...
  async function adminAvailable(){
    try{ const {data,error} = await supabase.from("stocks_available_for_admin").select("product,account_type,duration_code,total_qty").order("product"); if(error) throw error; if(data?.length) return data; } catch{}
  ...
  async function adminGetAccount(){
    ...
    // 1) Try RPC which should atomically decrement and return credentials
    setLoading(true);
    ...
    // 2) Fallback: pick one row and decrement
    if(error || !data?.length){
      // pick the oldest matching, unarchived, qty>0
