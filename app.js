...
(() => {
  ...
  async function boot(){
    ...
    // create supabase if config exists
    ...
    async function ownerAddStock(){
      if(!supabase) return alert("Missing config.js (Supabase).");
    ...
    async function ownerRenderRecords(){
      ...
      $$(".btnRecEdit", tbody).forEach(btn=>{
        btn.addEventListener("click", async ()=>{
          ...
          const cur={ product: tr.children[1].textContent.trim(), type: tr.children[2].textContent.trim(), expires_at: tr.children[4].textContent.trim(), buyer_link: tr.children[6].textContent.trim(), price: tr.children[7].textContent.trim() };
